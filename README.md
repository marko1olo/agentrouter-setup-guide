...
1. Перейдите на сайт [AgentRouter](https://agentrouter.org/register?aff=KM29) *(реф. ссылка с бонусом +$50)*.
2. Авторизуйтесь через свой аккаунт **GitHub**.
3. **Бонусы:** Вы получите **$125** за регистрацию через GitHub, **$50** бонусом за вход, и **$25** будет начисляться каждый день за вход в панель.
4. В левом меню перейдите в **API Keys** → **Create Key**.
5. Задайте имя (например, `vscode-key`) и скопируйте полученный токен (начинается на `sk-...`).
