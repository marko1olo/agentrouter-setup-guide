# 🔒 Security Policy — marko1olo/agentrouter-setup-guide
...
> Project Scope: `AgentRouter Multi-LLM Router & Fallback Orchestration Hub`
...
We actively maintain and provide critical security updates for the following release lines of **marko1olo/agentrouter-setup-guide**:
...
Security engineering in marko1olo/agentrouter-setup-guide is guided by the following domain-specific threat vector analyses:

### 1. API Key Exfiltration
* **Description**: Unsanitized debug endpoints leaking upstream LLM provider credentials.
...
### 2. DDoS via Recursive Routing
* **Description**: Circular fallback topologies causing runaway API request loops.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
### 3. MITM Attacks
* **Description**: Unverified SSL certificates on upstream proxy connections.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
...
If you discover a security flaw or exploit vector in **marko1olo/agentrouter-setup-guide**, do **NOT** post it publicly in open issues or discussions.
